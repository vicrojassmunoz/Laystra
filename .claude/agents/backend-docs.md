...
Objetivo: que el usuario (o cualquier instancia futura de Claude Code) pueda leer un solo fichero y entender qué hace el backend realmente, sin tener que leer cada módulo.
...
2. Actualiza `backend/docs/ARCHITECTURE.md` con:
   ...
   - Dónde vive el estado ahora mismo (ej. `InMemoryStore` en `app/services/store.py`, se resetea al reiniciar el servidor) — esto es fácil de olvidar y cambia de fase en fase, déjalo explícito y actualízalo cuando cambie (ej. cuando llegue SQLite en Fase 1).
