...
4. Devuelve un resumen conciso y accionable al hilo principal con esta plantilla:
...
## Ficheros clave
- `ruta` — qué hace / qué habría que tocar

## Contrato API (si aplica)
- Router/schema backend → api/types mobile → pantallas

## Dependencias cruzadas
(qué rompe si se cambia X)

## Riesgos / edge cases
(breves)

## Subagentes sugeridos
(ej. backend-coder → frontend-coder → verifier → reviewer)
```

Nada de código, nada de opiniones de diseño.
