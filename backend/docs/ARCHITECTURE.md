...
- `init_db()` — crea las tablas que falten vía `Base.metadata.create_all`, y a continuación llama a `_add_missing_columns()`. **Sigue sin haber Alembic**, pero desde que llegaron las super-series `init_db()` ya no es un `create_all` puro: ver "Migración ad-hoc de columnas" más abajo.
